...
namespace Maze
{
    public class PopulationManager : MonoBehaviour
    {
        ...
        [SerializeField]
        private GameObject m_BotPrefab = null;
        [SerializeField]
        ...
        private void Start()
        {
            ...
            {
                GameObject botInstance = SpawnBot();
        ...
        private void Update()
        {
            ...
            {
                if (AreAllBotsDead() || Input.GetKeyDown(KeyCode.N))
                {
                    StartNewTrial();
                }
            }
        ...
        private GameObject SpawnBot()
        {
            ...
            GameObject botInstance = Instantiate<GameObject>(m_BotPrefab);
            botInstance.transform.position = startingPos;
            botInstance.transform.rotation = startingRotation;

            return botInstance;
        }

        private bool AreAllBotsDead()
        {
            bool allDead = true;

            for (int index = 0; index < m_Population.Count && allDead; ++index)
            {
                GameObject botGo = m_Population[index];

                if (botGo == null)
                    continue;

                Brain brainComponent = botGo.GetComponent<Brain>();
        ...
        private void DestroyBots(List<GameObject> i_Bots)
        {
            if (i_Bots == null)
                return;

            for (int index = 0; index < i_Bots.Count; ++index)
            {
                GameObject bot = i_Bots[index];
        ...
        private void StartNewTrial()
        {
            ...
            for (int index = (int)(sortedPopulation.Count / 2f) - 1; index < sortedPopulation.Count - 1; ++index)
            {
                ...
                GameObject newBot02 = Breed(sortedPopulation[index + 1], sortedPopulation[index]);

                m_Population.Add(newBot01);
                m_Population.Add(newBot02);
            }

            DestroyBots(sortedPopulation);
        }
            ...
            if (i_Population != null && i_Population.Count > 0)
            {
                ...
                for (int firstIndex = 0; firstIndex < sorted.Count - 1; ++firstIndex)
                {
                    for (int secondIndex = firstIndex + 1; secondIndex < sorted.Count; ++secondIndex)
                    {
                        ...
                        int compare = CompareBots(first, second);
        ...
        private int CompareBots(GameObject i_Main, GameObject i_CompareTo)
        {
