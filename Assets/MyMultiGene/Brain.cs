...
namespace MyMultiGene
{
    ...
    public class Brain : MonoBehaviour
    {
        ...
        [SerializeField]
        private float m_TimeBetweenGene = 0f;
        ...
        private float m_NextGeneIndexTimer = 0f;
        private int m_CurrentGeneIndex = 0;
        ...
        private void FixedUpdate()
        {
            ...
            DNAGene firstGene = (DNAGene)m_DNA.GetGene(m_CurrentGeneIndex);
            ...
            if (m_TimeBetweenGene == 0f)
            {
                Internal_IncrementGeneIndex();
            }
            else
            {
                m_NextGeneIndexTimer += Time.fixedDeltaTime;

                if (m_NextGeneIndexTimer > m_TimeBetweenGene)
                {
                    Internal_IncrementGeneIndex();
                    m_NextGeneIndexTimer = 0f;
                }
            }
        }
        ...
        private void Internal_IncrementGeneIndex()
        {
            ++m_CurrentGeneIndex;
            if (m_CurrentGeneIndex == m_DNALength)
            {
                m_CurrentGeneIndex = 0;
            }
        }
